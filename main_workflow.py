...
# PR Information from workflow
REPO_OWNER = os.getenv("REPO_OWNER")
REPO_NAME = os.getenv("REPO_NAME")
PR_NUMBER = int(os.getenv("PR_NUMBER", 0))

if not all([GITHUB_TOKEN, GROQ_API_KEY, REPO_OWNER, REPO_NAME, PR_NUMBER]):
    print("❌ Missing required environment variables!")
    print(f"GITHUB_TOKEN: {'✓' if GITHUB_TOKEN else '✗'}")
    print(f"GROQ_API_KEY: {'✓' if GROQ_API_KEY else '✗'}")
    print(f"REPO_OWNER: {REPO_OWNER or '✗'}")
    print(f"REPO_NAME: {REPO_NAME or '✗'}")
    print(f"PR_NUMBER: {PR_NUMBER or '✗'}")
    sys.exit(1)
...
"""
    ...
    if pr_info['files']:
        ...
        # Add summary of file types if many files
...
def format_for_discord(pr_info, summary):
    ...
    embed = {
        ...
        "footer": {
            "text": f"PR #{PR_NUMBER} • {pr_info['basic']['state'].title()} • Auto-analyzed"
        }
    }
...
if __name__ == "__main__":
    try:
        print(f"🤖 GitHub PR Bot - Workflow Execution")
        print(f"📅 {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}")
        print(f"🔍 Analyzing PR #{PR_NUMBER} from {REPO_OWNER}/{REPO_NAME}")
        print("=" * 80)
        ...
        # Save analysis to file for workflow artifacts
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        try:
            with open(f'pr_analysis_{PR_NUMBER}_{timestamp}.txt', 'w', encoding='utf-8') as f:
                f.write(f"PR #{PR_NUMBER} Analysis - {REPO_OWNER}/{REPO_NAME}\n")
                f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}\n")
                f.write("="*80 + "\n\n")
                f.write(summary)
            print(f"📄 Analysis saved to pr_analysis_{PR_NUMBER}_{timestamp}.txt")
        except Exception as e:
